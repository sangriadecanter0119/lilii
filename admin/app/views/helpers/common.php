...
class CommonHelper extends AppHelper
{
  ...
  //入力オブジェクトがNULLなら&nbspを返す(文字列型)
  function evalSpaceForTimeKanji($obj)
  {
      $obj = trim($obj);
      return (is_null($obj) or empty($obj)) ? "":substr($obj,0,2)."時".substr($obj,3,2)."分";
  }
  ...
  //入力オブジェクトがNULLなら&nbspを返す(Date型:ショート)
  function evalNbspForShortDateKanji($obj)
  {
      return (is_null($obj) or empty($obj) or $obj == '0000-00-00 00:00:00') ? "&nbsp":date('Y年m月d日',strtotime($obj));
  }
  //入力オブジェクトがNULLなら&nbspを返す(Date型:ショート)
  function evalSpaceForShortDateKanji($obj)
  {
      return (is_null($obj) or empty($obj) or $obj == '0000-00-00 00:00:00') ? "":date('Y年m月d日',strtotime($obj));
  }
